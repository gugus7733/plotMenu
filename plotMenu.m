...
existingMenus = findall(groot, 'Tag', 'PlotMenuMainFigure');
...
leftPanel  = uipanel(mainLayout, 'Title', 'Data selection', 'BackgroundColor', theme.bgPanel, ...
    'Scrollable', 'on');
...
subplotPanel = uipanel(leftLayout, 'Title', 'Subplots', 'BackgroundColor', theme.bgPanel);
...
subplotLayout.BackgroundColor = theme.bgPanel;
...
actionsPanel = uipanel(leftLayout, 'Title', 'Plot actions', 'BackgroundColor', theme.bgPanel);
...
%% Center panel: axes
centerPanel  = uipanel(mainLayout, 'Title', 'Plot', 'BackgroundColor', theme.bgPanel);
...
rightPanel  = uipanel(mainLayout, 'Title', 'Style / Export', 'BackgroundColor', theme.bgPanel, ...
    'Scrollable', 'on');
...
appState.fontUiLight    = fontUiLight;
...
%% --- Nested helper and callback functions ---
    ...
    function applyTheme(appStateLocal)
        ...
        th = appStateLocal.currentTheme;
        
        % Select font profile based on theme
        if isequal(th, appStateLocal.themeDark)
            fontUi = appStateLocal.fontUiDark;
        else
            fontUi = appStateLocal.fontUiLight;
        end
        fontFig = appStateLocal.fontFigure;
        ...
        for pnl = appStateLocal.allPanels(:)'
            ...
            if isprop(pnl, 'BackgroundColor')
                pnl.BackgroundColor = th.bgPanel;
            end
            if isprop(pnl, 'ForegroundColor')
                pnl.ForegroundColor = th.fgText;
            end
            if isprop(pnl, 'FontName')
                pnl.FontName = fontUi.fontName;
            end
            if isprop(pnl, 'FontSize')
                pnl.FontSize = fontUi.fontSizeBase;
            end
        end
        
        if isgraphics(appStateLocal.axesScalePanel)
            appStateLocal.axesScalePanel.BackgroundColor = th.bgControlAlt;
        end
        
        if isgraphics(appStateLocal.mainLayout)
            appStateLocal.mainLayout.BackgroundColor = th.bgMain;
        end
        
        for layoutHandle = appStateLocal.allLayouts(:)'
            if isgraphics(layoutHandle) && isprop(layoutHandle, 'BackgroundColor')
                layoutHandle.BackgroundColor = th.bgPanel;
            end
        end
        
        altLayouts = appStateLocal.altLayouts;
        for layoutHandle = altLayouts(:)'
            if isgraphics(layoutHandle) && isprop(layoutHandle, 'BackgroundColor')
                layoutHandle.BackgroundColor = th.bgControlAlt;
            end
        end
        
        for h = appStateLocal.allButtons(:)'
            if ~isgraphics(h)
                continue;
            end
        ...
        for h = appStateLocal.allCheckboxes(:)'
            if ~isgraphics(h)
                continue;
            end
            if isprop(h, 'BackgroundColor')
                h.BackgroundColor = th.bgPanel;
            end
            if isprop(h, 'FontColor')
                h.FontColor = th.fgText;
            end
        ...
        for h = appStateLocal.allLabels(:)'
            ...
            if isprop(h, 'FontSize')
                h.FontSize = fontUi.fontSizeSmall;
            end
        end
        
        for h = [lblXTickSpacing, lblYTickSpacing, lblXScale, lblYScale]
            if isgraphics(h) && isprop(h, 'BackgroundColor')
                h.BackgroundColor = th.bgControlAlt;
            end
        end
        
        for h = appStateLocal.allEdits(:)'
            if ~isgraphics(h)
                continue;
            end
            if isprop(h, 'BackgroundColor')
        ...
        if isgraphics(btnColor)
            if isprop(btnColor, 'FontName')
                btnColor.FontName = fontUi.fontName;
            end
            if isprop(btnColor, 'FontSize')
                btnColor.FontSize = fontUi.fontSizeBase;
            end
            if isprop(btnColor, 'FontWeight')
                btnColor.FontWeight = 'bold';
            end
            if isprop(btnColor, 'FontColor')
                btnColor.FontColor = th.fgText;
            end
        end
        
        for h = appStateLocal.xListboxes(:)'
            if ~isgraphics(h)
                continue;
            end
            if isprop(h, 'BackgroundColor')
        ...
        if isgraphics(appStateLocal.hSubplotSelector)
            set(appStateLocal.hSubplotSelector, ...
                'BackgroundColor', th.bgControl, ...
                'FontName', fontUi.fontName, ...
                'FontSize', fontUi.fontSizeBase, ...
                'FontColor', th.fgText);
        end
        
        if isgraphics(lblLayoutSummary)
            lblLayoutSummary.FontWeight = 'bold';
            lblLayoutSummary.FontSize   = fontUi.fontSizeLarge;
        end
        if isgraphics(lblAxesScaleHeader)
        ...
        if isgraphics(lblStatus)
            ...
            lblStatus.FontName = fontUi.fontName;
            lblStatus.FontSize = fontUi.fontSizeBase;
        end
        
        % Force center panel (plots) to always use light theme for readability
        if isfield(appStateLocal, 'centerPanel') && isgraphics(appStateLocal.centerPanel)
            appStateLocal.centerPanel.BackgroundColor = appStateLocal.themeLight.bgPanel;
            appStateLocal.centerPanel.ForegroundColor = appStateLocal.themeLight.fgText;
        end
        
        if isfield(appStateLocal, 'centerLayout') && isgraphics(appStateLocal.centerLayout)
            appStateLocal.centerLayout.BackgroundColor = appStateLocal.themeLight.bgPanel;
        end
        
        % Apply figure font to all axes
        allAxes = findobj(appStateLocal.centerPanel, 'Type', 'axes');
        for ax = allAxes(:)'
            set(ax, 'FontName', fontFig.fontName, 'FontSize', fontFig.fontSize);
            set(ax.XLabel, 'FontName', fontFig.fontName, 'FontSize', fontFig.fontSize);
            set(ax.YLabel, 'FontName', fontFig.fontName, 'FontSize', fontFig.fontSize);
    ...
    function applyPanelStyle(panels, bgColor)
        ...
        for idxLocal = 1:numel(panels)
            ...
            if isgraphics(pnl)
                ...
                if isprop(pnl, 'FontName')
                    pnl.FontName = currentFont.fontName;
                end
                if isprop(pnl, 'FontSize')
                    pnl.FontSize = currentFont.fontSizeBase;
                end
            end
        end
    end
        ...
        for idxLocal = 1:numel(labels)
            ...
            if isgraphics(lbl)
                ...
                if isprop(lbl, 'FontName')
                    lbl.FontName = currentFont.fontName;
                end
                if isprop(lbl, 'FontSize')
                    lbl.FontSize = currentFont.fontSizeSmall;
                end
            end
        end
    end
    ...
    function applyControlStyle(controls, bgColor)
        ...
        for idxLocal = 1:numel(controls)
            ...
            if isgraphics(ctrl)
                ...
                if isprop(ctrl, 'FontName')
                    ctrl.FontName = currentFont.fontName;
                end
                if isprop(ctrl, 'FontSize')
                    ctrl.FontSize = currentFont.fontSizeBase;
                end
            end
        end
    end
        ...
        for idxLocal = 1:numel(checks)
            ...
            if isgraphics(chk)
                ...
                if isprop(chk, 'FontName')
                    chk.FontName = currentFont.fontName;
                end
                if isprop(chk, 'FontSize')
                    chk.FontSize = currentFont.fontSizeBase;
                end
            end
        end
    end
    ...
    function applyButtonStyle(buttons, useAltColor, makeBold)
        ...
        for idxLocal = 1:numel(buttons)
            ...
            if isprop(btn, 'FontName')
                btn.FontName = currentFont.fontName;
            end
            if isprop(btn, 'FontSize')
                btn.FontSize = currentFont.fontSizeBase;
            end
    ...
    function enterDerivedMode()
        ...
        if isequal(theme, themeDark)
            lbYVar.BackgroundColor = [0.15 0.25 0.35]; % Dark blue-ish for dark mode
        else
            lbYVar.BackgroundColor = [0.94 0.99 1]; % Light blue for light mode
        end
...
% end
    ...
    function onExport(~, ~)
        ...
        codeLines = {};
        codeLines{end+1} = '% Auto-generated by plotMenu';
